...
We can parallelise aspects of the computation across the submodels, possibly using less computationally expensive techniques for some submodels.
...
Note that the converse, that the minimiser of Equation \eqref{eqn:chained-marginal-replacement-kl} must be the marginal replacement proposed in Equation \eqref{eqn:chained-marginal-replacement}, is not necessarily true as there are many choices of $\pd_{\text{pool}}(\boldsymbol{\phi})$ that admit $\pd_{\text{pool}}(\phi_{\modelindex})$ as a marginal.

We form the chained melded model by taking the product of the marginally replaced submodels
...
- _I'm not sure I want to keep the following paragraphs / justification, but I feel that we need to justify extending the parameter space of each of the marginal replacements to include parameters that are irrelevant to each submodel._

We have chosen to define $\pd_{\text{pool}}(\boldsymbol{\phi})$ as a general function of all prior marginals, i.e.&nbsp; $\pd_{\text{pool}}(\boldsymbol{\phi}) = g(\pd_{1}(\phi_{1}), \pd_{2}(\phi_{2}), \ldots, \pd_{\modelindex}(\phi_{\modelindex}))$.
We now motivate this choice by discussing some less desirable alternatives.

One alternative would be to define
\input{tex-input/pooled-prior-discussion/0010-bad-alternative-one.tex}
However, this assumes that all the elements in $\boldsymbol{\phi}$ are a priori independent, which may not be plausible.
Another option is
\input{tex-input/pooled-prior-discussion/0020-bad-alternative-two.tex}
This option requires making $\Nm$ choices of pooling function, _and doesn't let us weight each of the $\pd_{\text{pool}}(\phi_{\modelindex})$ terms, which we might like to do? Not convinced_.
Thus the only form of $\pd_{\text{pool}}(\boldsymbol{\phi})$ that does not enforce independence between components, and requires only one choice about the form the pooling are the general forms of $g$ which we will now detail. (_circular_).
...
for nonnegative weight vector $\lambda$.
...
- _the $\phi_{\modelindex}$ notation obscures the **lack** of independence_.
...
However, no choice of $\lambda$ under either pooling method results in such a prior.

The one special case is $\Nm = 3$.
A definition for dictatorial pooling, which satisfies the aforementioned criteria, is a choice of either of the following $g$
\input{tex-input/multiple-phi/0057-dictatorial-pooling.tex}
as the marginal priors for $\phi_{1 \cap 2}$ and $\phi_{2 \cap 3}$ are associated with just one submodel each.
Equation \eqref{eqn:dictatorial-pooling-one} is a result of choosing $\lambda = (1, 0, 0, 1)$ in linear pooling, or $\lambda = (1, 0, 1)$ in logarithmic pooling.
Equation \eqref{eqn:dictatorial-pooling-two} can only arise by setting $\lambda = (0, 1, 0)$ under logarithmic pooling. 

### Pooling weights

Choosing values for the pooling weights ($\lambda$) is an important step in specifying $\pd_{\text{pool}}(\boldsymbol{\phi})$.
...
- The multi-stage samplers assume that the pooled prior decomposes such that $\pd_{\text{pool}}(\boldsymbol{\phi}) = \prod_{\modelindex = 1}^{\Nm}\pd_{\text{pool}, \modelindex}(\phi_{\modelindex})$.
  - _not quite right, we don't have to assume it (we can ignore the pooled prior until the end), but it is necessary to write a sampler than is actually multi-stage. otherwise, we necessarily have to sample $(\boldsymbol{\phi}, \psi_{1})$ when targeting submodel one in stage one._
  - _also not right in that what follows we need to assume that $\pd_{\text{pool}, \Nm}$ is a function of all elements of $\boldsymbol{\phi}$._
  - _I think that in the sequential case we're assuming that a decomposition exists such that $\pd_{\text{pool}}(\boldsymbol{\phi}) = \prod_{\modelindex = 1}^{\Nm}\pd_{\text{pool}, \modelindex}(\boldsymbol{\phi}_{1 : \modelindex})$, where $\boldsymbol{\phi}_{1 : \modelindex} = (\phi_{1}, \ldots, \phi_{\modelindex})$_.
  - _A different assumption is required for the parallel case._

- Our examples both contain $\Nm = 3$ submodels, so we only present posterior estimation methods for this case. (I can't figure out how to discuss the parallel sampler in the general $\Nm$ model case).
- The are a number of additional challenges for the general $\Nm$-submodel case. (probably for discussion)
  - You have to choose an order in which to consider the submodels, for both the sequential and parallel sampler.
    - greatly complicates the exposition and can impact the final result
    - complex in the sequential setting (can start with any submodel and incorporate any adjacent submodel until all are considered)
    - Very complex in the parallel setting (can start with any number of submodels and run arbitrarily ordered sequential samplers therein until some point I can't quite figure out -- I think its the step before any parallel process incorporates on of the models on the extremaof the subchain it is considering, provided that extrema is next to another subchain.) At some point this becomes D&C SMC.
...
- _we need a different pooled prior decomposition here_.
