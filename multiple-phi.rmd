...
_Partial dictatorial pooling_ enforces a single original submodel prior for the relevant components of $\boldsymbol{\phi}$, with no restrictions on the pooling of the remaining components; and _complete dictatorial pooling_ which requires selecting one of the two original priors for each component of $\boldsymbol{\phi}$.
...
Complete dictatorial pooling requires the marginal pooled prior for each component in $\boldsymbol{\phi}$ to be chosen solely on the basis of only one of the two priors specified for it under the original submodels.
...
If two consecutive marginals are chosen to have the same original prior then we instead define
