...
Rewriting the conditional distributions in terms of submodel joint densities and marginals yields
...
The corresponding melded posterior $\pd_{\text{meld}}(\phi_{1 \cap 2}, \phi_{2 \cap 3}, \psi_{1}, \psi_{2}, \psi_{3} \mid Y_{1}, Y_{2}, Y_{3})$ is proportional to Equation \eqref{eqn:melded-model-full}.
...
For simplicity we will also assume that $\phi_{1 \cap 2} \in \mathbb{R}^{d_{1}}$ and $\phi_{2 \cap 3} \in \mathbb{R}^{d_{2}}$, and the supports of $\pd_{1}, \pd_{2}$, and $\pd_{3}$ are $\mathbb{R}^{d_{1}}, \mathbb{R}^{d_{1} \times d_{2}}$, and $\mathbb{R}^{d_{2}}$ respectively, however none of our results are specific to marginals defined on these exact supports.
...
Specifying values for $\lambda = (\lambda_{1}, \lambda_{2}, \lambda_{3})$ that produce an appropriate $\pd_{\text{pool}}$ is a challenging problem [@genest:mcconway:90].
Using linear pooling to construct $\pd_{\text{pool}}$ allows us to interpret $\lambda$ as a vector that weights the contribution of each marginal density.
Interpreting the components of $\lambda$ as relative weights is standard when constructing mixture distributions[@everitt:hand:81]; often these weights are constrained to sum to 1, ensuring that the mixture density is normalised.
However, when employing logarithmic pooling, the components of $\lambda$ are not interpretable as weights in the usual, mixture distribution sense. 
This lack of interpretability is particularly apparent when $d_{1}$ and $d_{2}$ differ.
We would like to develop our intuition about the influence of $\lambda$ on $\pd_{\text{pool}}(\phi_{1 \cap 2}, \phi_{2 \cap 3})$, when using logarithmic pooling, so that our choice of $\lambda$ appropriately reflects prior information.
 
<!-- [^scope]: Recasting $\lambda$ as a parameter and estimating the corresponding melded posterior is (currently!) beyond the scope of this work. -->

## Pooled prior examples 

Specifying the type of pooling and values for the pooling weights is a subjective process.
The choices are sensitive to the availability of prior information, and the specifics of the marginals being pooled; providing  submodel-agnostic advice about the specification process is challenging.
The examples in this section demonstrate common challenges encountered when constructing the pooled prior, such as discovering which pooling method produces the desired pooled prior, and addressing the mismatch in supports between the marginals.
Our examples are necessarily simple, so that we can understand the impact of our choices on the pooled prior and provide generalisable advice on the specification process.

### Pooling Gaussian and Student-$t$ densities

#### Choosing relative values for the weights

Consider combining the following distributions using logarithmic pooling
\input{tex-input/multiple-phi/0061-marginal-gaussian-example.tex}
where $f(\phi; \mu, \sigma^2)$ is a density function with location parameter $\mu$ and scale parameter $\sigma$, and $f$ is an appropriate dimension Gaussian or Student-$t_{4}$ density function.
...
We see that logarithmic pooling has given us reasonable pooled priors for both values of $\lambda$. 
However it is difficult to understand the precise impact of the choice of $\lambda$ from these plots alone.

An analytic approach to understanding $\lambda$ may be more insightful.
We can precisely compute $\pd_{\text{pool}}$ in the Gaussian case, for arbitrary $\lambda$.
Noting that $f(\phi; \mu, \sigma^2)^{\lambda_{\modelindex}} = f(\phi; \mu, \frac{\sigma^2}{\lambda_{\modelindex}})$, we can write
...
This result aligns with the pooled densities shown in the left column of Figure \ref{fig:pooled_densities_plot}.
Correlation present in $\pd_{2}$ has changed the effect of $\lambda$.
In uncorrelated settings it only rescales marginal distributions; here we see it affecting the trade-off between $\pd_{2}$, and $\pd_{1}$ and $\pd_{3}$.
...
- I'm still not sure what I learnt from Figure \ref{fig:pooled_densities_plot}.
    - Densities move and concentrate when you pool them? Not hugely surprising?
...
- We could distribute weights between $(p_{1}(\phi_{1 \cap 2}) p_{3}(\phi_{2 \cap 3}))$ and $\pd_{2}(\phi_{1 \cap 2}, \phi_{2 \cap 3})$, as these are equal dimensional objects.
    ...
    - If we equally divide weights in this way, we have $\lambda_{1} + \lambda_{2} + \lambda_{3} > 1$, which is fine, but not exactly intuitive.
...
```
- The results in Equation \eqref{eqn:log-pooling-gaussian} are useful in 2 dimension, but what happens as $d_{1}$ and $d_{2}$ change, and grow further apart? 
- I think I want to find out how different density functions change as the dimension of their input increases
- To do so, we must evaluate $f(x; \mu, \sigma^2)$ for $x \in \mathbb{R}^{d}$ for increasing $d$.
- Denote the $d$-dimensional vector of zeros as $0_{d}$, and similarly vector of twos $2_{d}$.
- Figure \ref{fig:pooling_dimension_tests} shows the log of the density function, evaluate at these two values of $x_{d}$, as $d$ increases.
- In the Gaussian case, the density function decreases at an exponential rate.
    - The decrease occurs faster for a point further in the tails.
- In the Student-$t_{4}$ case, the density decreases exponentially for $x_{d} = 2_{d}$, i.e. a point sufficiently far in the tail of the distribution.
- However, for $x_{d} = 0_{d}$, the density function decreases, reaches a minimum, and then increases at a faster-than-exponential rate.
    - This behaviour makes it difficult to state the general impact of $\lambda$ in the pooled prior if one of the marginal densities is non-Gaussian / heavy tailed.

- _What do we learn here_:
- Choosing pooling weights in logarithmic pooling depends on:
    - Dimension of $\phi_{1 \cap 2}$ and $\phi_{1 \cap 3}$
    - Range of values of $\phi_{1 \cap 2}$ and $\phi_{1 \cap 3}$ that one intends to evaluate $\pd_{\text{pool}}$ over
    - Marginal density functions, in a way that is hard to generalise ($t_{\nu}$-distribution explodes in a  neighbourhood $0_{d}$ for certain $\nu, d$)
    - So really we're more certain that the only possible answer is 'it depends'?
...
Trace plots, rank plots, and numerical convergence measures [@vehtari:etal:diagnostics:19] for both stages of the parallel sampling process are presented in Appendix \ref{appendix-owls-example-diagnostics}.
...
- The melded quantities are not all root nodes in the DAG of the submodels
    - Need this for the suggested melded model here to be different from applying original melding twice
- Using PoE leads to an overconcentrated posterior
    - Would just be nice? Not required.
