...
There is widespread interest in integrating many sources of data into a single model, and some specific forms of combining data are well established.
...
Specifically, it considers $\Nm$ submodels that share some common quantity $\phi$, and for which each of the $\modelindex = 1, \ldots, \Nm$ submodels also has distinct parameters $\psi_{\modelindex}$, data $Y_{\modelindex}$ and form $\pd_{\modelindex}(\phi, \psi_{\modelindex}, Y_{\modelindex})$.
[@goudie_joining_2019] propose how to combine these submodels into a joint model $\pd_{\text{meld}}(\phi, \psi_{1}, \ldots, \psi_{\Nm}, Y_{1}, \ldots, Y_{\Nm})$.
...
_Dictatorial pooling_ is a special case of either pooling method when $\lambda_{\modelindex'} = 1$ and, for all $\modelindex eq \modelindex’$, $\lambda_{\modelindex} = 0$.
