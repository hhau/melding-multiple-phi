...
- We should set the event time to $t = 1$ (the maximum possible time) when the event is censored (i.e. $T_{i}^{*} = C_{i} = 1$ when $\delta_{i} = \mathcal{I} \{T_{i}^{*} < C_{i}\} = 0$)
...
Using Equation (28) from @brilleman:etal:20, the log likelihood is
...
- melding seems to be doing a very similar thing to the correction proposed in https://arxiv.org/pdf/1808.07719.pdf in Eq (6)
