...
It is unclear how to integrate models where there is no single quantity $\phi$ common to all submodels.
We consider submodels that are linked in a chain structure.
Specifically, in the $\Nm = 3$ case, submodel 1 and 2 share a common quantity $\phi_{1 \cap 2}$, and submodel 2 and 3 share a different quantity $\phi_{2 \cap 3}$. 
We propose an extension to Markov melding, which we call _chained Markov melding_[^chained], which facilitates the combination of submodels that share quantities in this pairwise manner.
...
By considering the submodel as the minimum computational unit, we can parallelise aspects of the computation, and use less computationally expensive techniques for some submodels.
...
We illustrate our methodology using two examples, for which we now provide a high-level overview.
Our examples show that chain like inter-submodel relationships arise in many applied settings
...
No single source of data is sufficient to estimate all quantities of interest.
Thus it is necessary to integrate the three submodels into a single joint model, in order to produce acceptably precise estimates of fecundity and immigration rates.
We will show that the chained Markov melding framework developed in Section \ref{model-specification} encapsulates the process of integrating these submodels, and IPMs in general.
...
#### If we go with the example motivation

- _the numbers in the example need changing to match PF ratios, and we need to justify our use of simulated data / which covariate we are thinking of as longitudinal_.
- _I also don't actually think threshold regression is very relevant. It is a competitor to PH cox models, not those with time varying covariates, and is other wise much the same as standard survival analysis, using a slightly wider variety of baseline hazards (derived from the stochastic process origins)_.
...
A patient can be said to be experiencing ARDS if the ratio of the partial pressure of arterial blood oxygen (Pao\textsubscript{2}) to the faction of inspired oxygen (Fio\textsubscript{2}) is less than 300mmHg [@the_ards_definition_task_force_acute_2012].
The laboratory test results required to calculate a patient's Pao\textsubscript{2}/Fio\textsubscript{2} ratio are not always available.
An alternative is to consider the ratio of peripheral oxygen saturation (Spo\textsubscript{2}) to Fio\textsubscript{2}, as the former can be obtained from a pulse oximeter, a device typically worn on the finger tip that produces immediate measurements.
The Spo\textsubscript{2}/Fio\textsubscript{2} ratio can then be converted to a Pao\textsubscript{2}/Fio\textsubscript{2} by various formula [@pandharipande_derivation_2009-1; @brown_nonlinear_2016-1].
However, the noise introduced by the Spo\textsubscript{2} measurements [@nitzan_pulse_2014] and the conversion formula yield a Pao\textsubscript{2}/Fio\textsubscript{2} value that is contaminated by statistically nonneglible amounts of noise.
...
Chained Markov melding offers a conceptually straightforward, Bayesian approach to incorporating uncertain event times.
...
Consider $\modelindex = 1, \ldots, \Nm$ submodels each with data $Y_{\modelindex}$ and parameters $\theta_{\modelindex}$ denoted $\pd_{\modelindex}(\theta_{\modelindex}, Y_{\modelindex})$.
...
For notational convenience define $\phi_{1} = \phi_{1 \cap 2}, \phi_{\Nm} = \phi_{\Nm-1 \cap \Nm}$ and $\phi_{\modelindex} = \phi_{\modelindex - 1 \cap \modelindex} \cup \phi_{\modelindex \cap \modelindex + 1}, \modelindex = 2, \ldots, \Nm - 1$, so that $\phi_{\modelindex}$ denotes the parameters in model $\modelindex$ shared with another model.
The model-specific parameters of model $\modelindex$ are thus $\psi_\modelindex = \theta_\modelindex \setminus \phi_\modelindex$.

We focus in the main text on the $\Nm = 3$ submodel special case, where submodels $\pd_{1}(\phi_{1 \cap 2}, \psi_{1}, Y_{1})$ and $\pd_{2}(\phi_{1 \cap 2}, \phi_{2 \cap 3}, \psi_{2}, Y_{2})$ have $\phi_{1 \cap 2}$ in common, and $\pd_{2}(\phi_{1 \cap 2}, \phi_{2 \cap 3}, \psi_{2}, Y_{2})$ and $\pd_{3}(\phi_{2 \cap 3}, \psi_{3}, Y_{3})$ both contain $\phi_{2 \cap 3}$; Appendix \ref{general-case-of-the-chained-melded-model-and-pooled-prior} contains the general $\Nm$-model case.
...
Goudie _et al._&nbsp;show that $\pd_{\text{repl}, \modelindex}$ minimises the Kullback–Leibler (KL) divergence between a distribution $\q(\phi, \psi_{\modelindex}, Y_{\modelindex})$ and $\pd_{\modelindex}(\phi, \psi_{\modelindex}, Y_{\modelindex})$ under the constraint that $\q(\phi) = \pd_{\text{pool}}(\phi)$, and that marginal replacement is valid when $\phi$ is a deterministic function of the other parameters in submodel $\modelindex$.
...
## Chained Markov melding_

-_this is not correct at the moment_. 
...
For $\pd_{1}(\phi_{1 \cap 2}, \psi_{1}, Y_{1})$ and $\pd_{3}(\phi_{2 \cap 3}, \psi_{3}, Y_{3})$, marginal replacement results in
\input{tex-input/multiple-phi/0001-chained-marginal-replacement.tex}
where $\pd_{\text{pool}}(\phi_{1 \cap 2}, \phi_{2 \cap 3}) = g(\pd_{1}(\phi_{1 \cap 2}), \pd_{2}(\phi_{1 \cap 2}, \phi_{2 \cap 3}), \pd_{3}(\phi_{2 \cap 3}))$ is a pooling function that appropriately summarises all submodel prior marginals, $\pd_{\text{pool}}(\phi_{1 \cap 2}) = \int \pd_{\text{pool}}(\phi_{1 \cap 2}, \phi_{2 \cap 3}) \text{d} \phi_{2 \cap 3}$ and equivalently for $\pd_{\text{pool}}(\phi_{2 \cap 3})$.
The final equalities in Equations \eqref{eqn:chained-marginal-replacement-one} and \eqref{eqn:chained-marginal-replacement-two} hold because $\pd_{\text{repl}, 1}(\phi_{1 \cap 2}, \psi_{1}, Y_{1})$ is, by definition, independent of $\phi_{2 \cap 3}$ and likewise $\pd_{\text{repl}, 3}(\phi_{2 \cap 3}, \psi_{3}, Y_{3})$ does not depend on $\phi_{1 \cap 2}$.
Applying the marginal replacement procedure to the second submodel yields
\input{tex-input/multiple-phi/0002-submodel-two-marginal-replacement.tex}
The marginal replacements retain the attractive properties of the original marginal replacement procedure.
Specifically, they permit one or both of $\phi_{1 \cap 2}$ and $\phi_{2 \cap 3}$ to be the result of a deterministic function of other submodel parameters, and the following KL divergences are minimised
\input{tex-input/multiple-phi/0003-chained-marginal-replacement-kl.tex}
Finally, we form the chained melded model by taking the product of the marginally replaced submodels
...
As in the Markov melding setting, we would like to choose the pooling function $g$ to appropriately represent prior knowledge about the common quantities.
This is not a trivial task given the many possibilities for pooling functions and pooling weights, which we detail in the following sections.
Hence, it is important to assess the ensuing pooled prior is indeed an appropriate summary of prior knowledge, and we recommend producing prior predictive visualisations of the pooled prior as part of a Bayesian workflow [@gelman_bayesian_2020].

A complication encountered in the chained melding setting is that Equations \eqref{eqn:orig-pooling-linear} and \eqref{eqn:orig-pooling-logarithmic} are not immediately applicable.
The interpretation Equation \eqref{eqn:orig-pooling-linear} is particularly unclear when the submodels marginal distributions are of different parameters.
...
The first step forms intermediary pooling densities via standard linear pooling, using appropriate marginals the relevant quantity
\input{tex-input/multiple-phi/0054-silly-linear-solution.tex}
where $\lambda = (\lambda_{1, 1}, \lambda_{1, 2}, \lambda_{2, 1}, \lambda_{2, 2})$ is vector of nonnegative pooling weights.
We discuss the choice of $\lambda$ in the follow section.
...
Clearly, this prior assumes a priori independence between $\phi_{1 \cap 2}$ and $\phi_{2 \cap 3}$ which may be undesirable, particularly if this independence was not present under $\pd_2(\phi_{1 \cap 2}, \phi_{2 \cap 3})$.
...
for nonnegative weights $\lambda = (\lambda_{1}, \lambda_{2}, \lambda_{3})$.
One advantage of logarithmic pooling is that does not necessarily imply a priori independence between $\phi_{1 \cap 2}$ and $\phi_{2 \cap 3}$.
When $\lambda_{1} = \lambda_{2} = \lambda_{3} = 1$ we obtain the special case of Product-of-experts pooling [@hinton_training_2002]. 

#### Dictatorial pooling 

Chained Markov melding does not admit a direct analogue to the dictatorial pooling notion of Section \ref{pooled-prior} because not all submodel prior marginals contain all common quantities.
Indeed dictatorial pooling in the chained melding setting is not well defined[^dictdef] for $\Nm > 3$.
However, in the $\Nm = 3$ we propose that a sensible definition of dictatorial pooling is a choice of $g$ such that
\input{tex-input/multiple-phi/0057-dictatorial-pooling.tex}
as the marginal priors for $\phi_{1 \cap 2}$ and $\phi_{2 \cap 3}$ are associated with just one submodel.
Equation \eqref{eqn:dictatorial-pooling-one} can result from choosing $\lambda = (1, 0, 0, 1)$ in linear pooling, or $\lambda = (1, 0, 1)$ in logarithmic pooling.
Equation \eqref{eqn:dictatorial-pooling-two} can only arise by setting $\lambda = (0, 1, 0)$ under logarithmic pooling.

[^dictdef]: Consider $\Nm = 4$. It is not possible to chose a subset of $\pd_{1}(\phi_{1 \cap 2}), \pd_{2}(\phi_{1 \cap 2}, \phi_{2 \cap 3}), \pd_{3}(\phi_{2 \cap 3}, \phi_{3 \cap 4}), \pd_{4}(\phi_{3 \cap 4})$ that results in marginal priors for $(\phi_{1 \cap 2}, \phi_{2 \cap 3}, \phi_{3 \cap 4})$ that are all specific to only one submodel.

### Pooling weights

Choosing values for the pooling weights ($\lambda$) is an important step in specifying $\pd_{\text{pool}}$.
Figure \ref{fig:pooled_densities_plot} illustrates how $\lambda$ and the choice of pooling method impacts $\pd_{\text{pool}}$ when pooling normal distributions.
Specifically, we pool
...
Note the lack of correlation in $\pd_{\text{pool}}$ under linear pooling, due to \eqref{eqn:silly-linear-overall}.
...
Completing the specification of $\pd_{\text{meld}}$ requires choosing $\pd_\text{pool}(\phi_{1 \cap 2}, \phi_{2 \cap 3})$.
