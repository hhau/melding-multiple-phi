...
where each $\lambda_{\modelindex} \in \mathbb{R}^{+}$ is a submodel specific pooling weight, however we do not require that the weights sum to 1. 
...
Because we do not require that $\lambda_{1, 1} + \lambda_{2, 1} = 1$, $\pd_{\text{pool}, 1}$ is not necessarily normalised by construction; likewise for $\pd_{\text{pool}, 2}$.
...
Specifying values for $\lambda = (\lambda_{1}, \lambda_{2}, \lambda_{3})$ that produce an appropriate $\pd_{\text{pool}}$ is challenging.
Using linear pooling to construct $\pd_{\text{pool}}$ allows us to interpret $\lambda$ as a vector that weights the contribution of each marginal density [@genest:mcconway:90].
...
The examples in this section we apply common challenges encountered when constructing the pooled prior, such a. for use in Markov ering which pooling method produces the desired pooled prior, and addressing the mismatch in supports between the marginals## Pooling Gaussian and Student-$t$ densities

#### Choosing relative values for the weights

We first investigate the choice of pooling method and value of $\lambda$.\input{tex-input/multiple-phi/0061-marginal-gaussian-example.tex}
...
Our previous experiments pertain to the $d_{1} = d_{2} = 1$ case.
However, the intuition obtained in this low dimensional setting does not generalise to higher dimensional ones, which makes choosing an appropriate $\lambda$ more challenging.
To illustrate how different high dimensional settings can be, we consider $d$-dimensional versions of the Gaussian and Student-$t_{4}$ densities, with $d$ dimensional zero vector $0_{d}$ and identity matrix $\mathcal{I}_{d}$ as mean/location and covariance/scale respectively.
...
The sampler described in this section we apply _sequential_
Stage one ($s_{1}$) target. for use in Markov melding terms from submodel $\pd_{1}$. The target is expanded in stage two ($s_{2}$) to also include $\pd_{2}$ ly to include the $\pd_{3}$ terms and $\pd_\text{pool}$ in stage three ($s_{3}$)### Stage one
