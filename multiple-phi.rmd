...
In our second example we consider a joint model for longitudinal and survival data, with the added complication that the event times and outcomes are uncertain, model derived quantities.

Joint models for longitudinal and survival data [@rizopoulos_joint_2012] are popular models when longitudinal, often biomarker, data is available concurrently with event data.
...
This motivates a two-stage approach, were the longitudinal model is fit first and its output is used in the event model [@tsiatis_joint_2004; @mauff_joint_2020].

In some settings the time at which the relevant event occurs is inherently uncertain.
This can be due to the definition of the event, for example the time when a measurable but noisy quantity crosses a prescribed threshold, or because the event of interest is inherently unobservable and must be inferred or diagnosed from other information.
Examples include kidney failure [@diggle_real-time_2015]
...
- Why ?:
    ...
    Integrating the output of such models into standard parametric or Cox proportional hazards survival models is challenging, but, we argue, made conceptually and computational easier by the chained melding framework of Section \ref{model-specification}.
...
In addition, we censor the events at time $t = 1$ such that we only record event times $T_{i} = \min(T^{*}_{i}, 1)$ and event indicators $\delta_{i} = \boldsymbol{1}_{\{T^{*}_{i} \in (0, 1)\}}$. 
...
This compels us to consider this submodel in stage 1 when estimating the chained melded posterior.
...
Because we have multiple models for the common quantities, the joint model obtained via the product of the independent submodel is not generative.
...
- how is this estimated
 ...
 _Need to think about where to put this?_ -> I think it should go here.
...
- fixing event time underestimates the true uncertainty and introduces bias.
- Fixing the longitudinal submodel seems to have minimal effect, but this is probably due to the simplicity of the model. It is well known that the longitudinal submodel in a joint model can markedly influence the posterior of the survival model, particularly if it is poor fit to the longitudinal data.

# Discussion

_what have we introduced_

- We have introduced the chained melded model, and illustrated the process by which multiple sources of information are combined to obtained the melded posterior.
- Our two examples demonstrate that practitioners are already employing modelling processes that combine multiple sources of information, and that ignoring the uncertainty in any submodel component can produce biased, over-confident inference.
- We note one interesting contribution of this work is that it makes explicit the, often informal, process of summarising and approximating models for use in subsequent analyses. 
...
_approx strategies as they relate to other inference methods_

- We also detail two approximation strategies, that we believe are used in practice a great deal.
    1. The first approximates the subposterior of the common quantity with a Normal approximation, which is then used as a prior in stage two,
    2. The second is simpler, taking only a point estimate of the subposterior, and treating it as data in stage two.
- These strategies may admit acceptable approximations to the chained melded model, but in some instances they will produced biased and over-confident inference.
- It is  interesting to note that many 'preprocessing' steps in an analysis pipeline can be thought of as 'summary by point estimate', the latter strategy, where data are denoised or otherwise cleaned via models, for future use. 
- Such strategies are implementations of 'multi-phase' inference [@lin_trio_2014] 
