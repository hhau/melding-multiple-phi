...
This motivates a two-stage approach, where the longitudinal submodel is fit first and its output is used in the event submodel [@tsiatis_joint_2004; @mauff_joint_2020].
...
Another setting in which inherently uncertain event times can arise is record linkage [@harron_methodological_2015], and incorporating the uncertainty into standard survival models is not straightforward [@wang_integrative_2020].

It seems desirable to integrate both uncertain event times and longitudinal data in a survival analysis; modular inference methods, and chained Markov melding in particular, offer conceptually straightforward approaches.
Specifically, we consider the event time as an uncertain submodel output from a simplified threshold crossing model, which we denote $\pd_{1}$, and as the response in the survival model $\pd_{2}$. 
