...
Figure \ref{fig:pf_fit_and_fluid_fit_plot} displays the P/F data, the fitted submodel, and derived event times for individuals $i = 6, 15, 17$, and $29$.
...
It is interesting to see the relatively wide, multimodal distribution for $\{T_{29}, d_{29}\}$ (and for some other individuals not shown here).
...
Figure \ref{fig:pf_fit_and_fluid_fit_plot} displays the cumulative fluid data and the fitted submodel for individuals $i = 6, 15, 17$, and $29$.
...
[^fluidproperty]: This property is also true of the $\pd_{3}$.
...
\begin{equation}
\begin{aligned}
\alpha\left(&\{\theta_{1, n}^{*}, \psi_{1, n}^{*}\}, \{\theta_{1, n}, \psi_{1, n}\} \right)  \\
= &\frac {
  \pd_{1} \left(\theta_{1, n}^{*}, \psi_{1, n}^{*} \mid Y_{1} \right)
} {
  \pd_{1} \left(\theta_{1, n}, \psi_{1, n} \mid Y_{1} \right)
