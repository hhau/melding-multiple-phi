...
_first hitting time_ models [@lee_threshold_2006] are used for such data, and applications of threshold crossing models include kidney failure [@diggle_real-time_2015], dementia diagnosis [@hashemi_latent_2003], and disease transmission [@sweeting_estimating_2010; @yu_semi-parametric_2009].

We consider the event time as an uncertain submodel output from a simplified threshold crossing model, which we denote $\pd_{1}$, and as the response in the survival model $\pd_{2}$. 
...
This permits the use of the parallel sampler described in Section \ref{parallel-sampler}.
...
We note one contribution of this work is to make explicit the, often informal, process of summarising and approximating submodels for use in subsequent analyses.
