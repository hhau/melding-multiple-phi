...
Instead, it may be preferable to approximate such a joint model by joining the individual submodels together. 
...
Meta-analyses and evidence synthesis methods are important modelling techniques that summarise many disparate data, often by complex, hierarchical joint models [@ades_multiparameter_2006; @presanis_synthesising_2014].
Outside of the statical literature, a common name for combining multiple data is _data fusion_ [@lahat_multimodal_2015-1; @kedem_statistical_2017], though there are many distinct methods that fall under this general name.
...
- both the FHT and threshold regression models are inseparable from the stochastic process definition of the first submodel. We could recast our first submodel as some kind of parametric homage to a FHT/TR model, but nothing more than that. 
