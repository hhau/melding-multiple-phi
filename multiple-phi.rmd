...
The second example is a realistic and complex setting in which the combining of submodels without chained Markov melding is non-obvious.
...
Code and data for this example, as well as trace plots and numerical convergence measures [@vehtari_rank-normalization_2020] for both stages of the parallel sampling process, are available in the accompanying online repository [https://doi.org/10.5281/zenodo.5717455](https://doi.org/10.5281/zenodo.5717455).
...
Visual and numerical diagnostics [@vehtari_rank-normalization_2020] are assessed and are available in the repository accompanying this paper [https://doi.org/10.5281/zenodo.5717455](https://doi.org/10.5281/zenodo.5717455).
...
The nonsymmetric effect of the transformation from the log scale also implies that symmetric priors are not obviously sensible.
...
We assess the fit of this estimate by drawing samples from $\widehat{\pd}_{2, i}(\phi_{1 \cap 2}, \phi_{2 \cap 3})$ and comparing them to the Monte Carlo samples drawn using `simsurv`.
...
1. Each ICU stay must have at least 12 \pfratio\ observations ($J_{i} \geq 12$), with the first 6 being greater than 350 ($z_{i, j} > 350$ for $j = 1, \dots, 1$).
...
1. The time between any 2 consecutive \pfratio\ observations cannot exceed 2 days.
    - This is because we believe this lack of \pfratio\ observations is likely to be an error in the data: e.g. what appears as a single ICU stay in the data is actually two or more separate stays.
...
1. There must be sufficient temporal overlap between the fluid data and the \pfratio\ data. Specifically,
  \begin{equation}
    \frac{
