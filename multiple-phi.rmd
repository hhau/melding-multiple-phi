...
@schaub:etal:06 and @abadi:etal:10 consider aim to estimate fecundity, immigration, and yearly survival rates for a population of little owls.
They collect three sources of data, namely nest-record, population count, and capture-recapture.
Each of these data inform fecundity, immigration, and yearly survival rates respectively.
...
The initial population size $[x_{J, 1}, x_{A, 1}]$ is a priori uniformly distributed over $\{0, 1, \ldots, 50\}$.
...
Denote $\phi_{2 \cap 3} = \rho$, $\psi_{3} = \varnothing$, and $Y_{3} = \{(N_{t}, n_{t})\}_{t = 1}^{T}$ for consistency with the notation introduced in Section \ref{melded-model}.
 
#### Parameterisation

@abadi:etal:10 opt to parameterise the quantities via linear predictors, to minimise the number of parameters in the submodels.

- $\text{logit}(\phi_{a, s, t}) = \alpha_{0} + \alpha_{1}\mathbb{I}(s = M) + \alpha_{2}\mathbb{I}(a = A)$
- $\text{logit}(\pi_{s, t}) = \alpha_{4} \mathbb{I}(s = M) + \alpha_{5, t}$
- $\log(\eta) = \alpha_{6}$

The quantities common to the submodels are thus:

- $\phi_{1 \cap 2} = (\alpha_{0}, \alpha_{2})$,
- $\phi_{2 \cap 3} = \rho$.

Note that we do not include $\alpha_{1}$ as this is male specific, and hence does not exist in the count data model ($\pd_{2}$).

### Sampling

- We use the split and combine sampler.
- Results are compared to a long run of the original IPM code, which we treat as a baseline truth.
- Results obtained from this sampler are identical, see Figure \ref{fig:phi_subpost} for posterior credible intervals, and Figure \ref{fig:phi_qq_compare} for a QQ plot of the melded quantities.
