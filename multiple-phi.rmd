...
abstract: |
  ...
  One challenge in the joining process is appropriately capturing the prior dependence between common quantities within a submodel, whilst also reconciling differences in priors for the same common quantity between two adjacent submodels.
...
We will show that the chained Markov melding framework developed in Section \ref{chained-model-specification} encapsulates the process of integrating these submodels.
...
The first submodel fits a B-spline to the \pfratio\ data to calculate if and when an individual starts experiencing respiratory failure.
...
Individuals experience respiratory failure ($d_{i} = 1$) at arbitrary time $0 < t < C_{i}$, or are censored $(d_{i} = 0, t = C_{i})$.
...
Because we prefer the chained melded posterior, under either pooling method, for its full accounting of uncertainty we conclude that $\pd(\alpha \mid, \widehat{\pd}_{1 \cap 2}, \widehat{\pd}_{2 \cap 3}, Y_{2})$ is both overconfident and biased.
...
The melding methodology is general and permits any form of uncertainty in the common quantities.
