...
Clearly, this assumes a priori independence amongst all components of $\boldsymbol{\phi}$ which may be undesirable, particularly if this independence was not present in any particular marginal $\pd_{\modelindex}(\phi_{\modelindex - 1 \cap \modelindex}, \phi_{\modelindex \cap \modelindex + 1})$.
...
All pooled priors trivially satisfy \eqref{eqn:sequential-sampler-decomposition}, as we can assume all but $\pd_{3}(\phi_{1 \cap 2}, \phi_{2 \cap 3})$ are improper, flat distributions.
...
In general, all $\widehat{\pd}(\cdot \mid \widehat{\mu}, \widehat{\Sigma})$ terms are normal approximations, each with distinct means and covariances.
...
```{r phi_subpost_melding_only, fig.cap = 'Figure \\ref{fig:phi_subpost} without the individual subposteriors from submodels $\\pd_{1}, \\pd_{2}$, and $\\pd_{3}$. Different choices of pooling methods and pooling weights produce different posterior estimates for $\\alpha_{0}$ and $\\alpha_{2}$, but do not affect $\\rho$.'}
knitr::include_graphics('plots/owls-example/subposteriors-melding-only.pdf')
```
...
Trace plots, rank plots, and numerical convergence measures [@vehtari_rank-normalization_2020] for both stages of the parallel sampling process are presented in Appendix \ref{diagnostics-for-the-owls-example}.
...
To completely align with our chained melding notation we also define $Y_{1} = (\{\boldsymbol{z}_{i}, \boldsymbol{t}_{i}\}_{i = 1}^{N})$ and $\psi_{1} = (\{\omega_{i}\}_{i = 2}^{N})$.
...
\end{aligned}
\end{equation}
we would like to update $\phi_{1 \cap 2}$ by updating $(T_{i}, d_{i})$ for one individual at a time, for a total of $N$ 'sub-steps'.
...
Our target is
\begin{equation}
  \pd_{1}(\theta_{1, 1}, \psi_{1} \mid Y_{1}, \{\theta_{1, i}\}_{i = 2}^{N})
...
\begin{equation}
  \frac {
    \pd_{1}(\theta_{1, 1}, \psi_{1}, \{\theta_{1, i}\}_{i = 2}^{N}, \mid Y_{1})
  } {
    \pd_{1}(\{\theta_{1, i}\}_{i = 2}^{N} \mid Y_{1})
  }
  \frac {
...
We propose $\psi_{1}^{*}$ and $\theta_{1, 1}^{*}$ (hence $\phi_{1 \cap 2, 1}^{*})$ by sampling a random integer from $\{1, \ldots, K\}$, retrieving the corresponding values of $\psi_{1}$ and $\theta_{1}$, and ignoring $(\theta_{1, 2}, \ldots, \theta_{1, N})$.
The marginal we are actually proposing from is
\begin{equation}
  \pd_{1}(\theta_{1, 1}, \psi_{1} \mid Y_{1})
  = \int \pd_{1}(\theta_{1}, \psi_{1} \mid Y_{1}) \text{d} (\{\theta_{1, i}\}_{i = 2}^{N})
\end{equation}
This integral is analytically intractable, however we know[^assumption] that $\pd_{1}(\theta_{1, 1}, \psi_{1} \mid Y_{1}) \propto \pd_{1}(\theta_{1}, \psi_{1} \mid Y_{1})$ where the constant of proportionality does not depend on $(\theta_{1, 1}, \psi_{1})$.

[^assumption]: TODO: This might actually be an assumption? I seem to think that it is true and assume it to be true in my notes. Now I am not sure this is true. Argh. I think this is true if we also condition on $\psi_{1}$, because then all the subject specific parameters are conditionally independent? No, this doesn't help, we're still assuming that the marginal is proportional to the joint.

Under this proposal, noting that the denominator terms in \eqref{eqn:sub-step-one-target} do not depend on the proposed parameters, the acceptance probability for this sub-step is
\begin{equation}
\begin{aligned}
\alpha&\left(\{\theta_{1, 1}^{*}, \psi_{1}^{*}\}, \{\theta_{1, 1}, \psi_{1}\} \right) \\
&=
\frac {
  \pd_{1}(\{\theta_{1, 1}^{*}\}, \{\theta_{1, i}\}_{i = 2}^{N}, \psi_{1}^{*} \mid Y_{1})
...
At sub-step $n$, for $1 < n \leq N$ with $\{\theta_{1, i}\}_{i = N}^{N} = \varnothing$, we have updated $\psi_{1}$ to $\psi_{1}^{[t]}$ and $(\{\theta_{1, i}\}_{i = 1}^{n - 1})$ to $(\{\theta_{1, i}^{[t]}\}_{i = 1}^{n - 1})$.
Thus the target is
\begin{multline}
  \pd_{1} \left(\theta_{1, n} \mid Y_{1}, \psi_{1}^{[t]}, \{\theta_{1, i}^{[t]}\}_{i = 1}^{n - 1}, \{\theta_{1, i}\}_{i = n + 1}^{N}\right)
  \pd_{2} \left(\phi_{1 \cap 2, n}(\theta_{1, n}) \mid Y_{2}, \psi_{2}, \phi_{2 \cap 3}, \{\phi_{1 \cap 2, i}(\theta_{1, i}^{[t]})\}_{i = 1}^{n - 1}, \{\phi_{1 \cap 2, i}(\theta_{1, i})\}_{i = n + 1}^{N}\right) \\
  = \frac{
    \pd_{1} \left(\theta_{1, n}, \psi_{1}^{[t]}, \{\theta_{1, i}^{[t]}\}_{i = 1}^{n - 1}, \{\theta_{1, i}\}_{i = n + 1}^{N} \mid Y_{1} \right)
  } {
    \pd_{1} \left(\psi_{1}^{[t]}, \{\theta_{1, i}^{[t]}\}_{i = 1}^{n - 1}, \{\theta_{1, i}\}_{i = n + 1}^{N}\mid Y_{1}\right)
  }
  \frac {
...
\end{multline}
We propose $\theta_{1, n}^{*}$ by drawing a random sample from $\pd_{1}(\theta_{1}, \psi_{1} \mid Y_{1})$ and ignoring all the other components.
The proposal distribution is $\pd_{1}(\theta_{1, n}, \mid Y_{1})$ and we again must assume that it is proportional to $\pd_{1}(\theta_{1}, \psi_{1} \mid Y_{1})$.
It is useful to note for a fixed value of $\psi_{1}$
\begin{equation}
  \pd_{1}(\theta_{1}, \psi_{1} \mid Y_{1}) =
  \prod_{i = 1}^{N}\pd_{1}(\theta_{1, i} \mid \psi_{1}, Y_{1}) \pd_{1}(\psi_{1} \mid Y_{1})
  \propto \pd_{1}(\theta_{1, n} \mid \psi_{1}, Y_{1}),
\end{equation}
so the equivalent assumption is $\pd_{1}(\theta_{1, n}, \mid Y_{1}) \propto \pd_{1}(\theta_{1, n} \mid \psi_{1}, Y_{1})$, which seems unlikely?

Ignoring these issues for the moment.
The acceptance probability, again noting that the denominators in \eqref{eqn:two-to-N-target} are constant, is
\begin{equation}
\begin{aligned}
\alpha\left(\{\theta_{1, n}^{*}\}, \{\theta_{1, n}\} \right) =
&\frac {
  \pd_{1} \left(\theta_{1, n}^{*}, \psi_{1}^{[t]}, \{\theta_{1, i}^{[t]}\}_{i = 1}^{n - 1}, \{\theta_{1, i}\}_{i = n + 1}^{N} \mid Y_{1} \right)
} {
  \pd_{1} \left(\theta_{1, n}, \psi_{1}^{[t]}, \{\theta_{1, i}^{[t]}\}_{i = 1}^{n - 1}, \{\theta_{1, i}\}_{i = n + 1}^{N} \mid Y_{1} \right)
...
& \times \frac{
  \pd_{1}(\{\theta_{1, n}\}, \{\theta_{1, i}\}_{i = 1}^{n - 1}, \{\theta_{1, i}\}_{i = n + 1}^{N}, \psi_{1} \mid Y_{1})
} {
  \pd_{1}(\{\theta_{1, n}^{*}\}, \{\theta_{1, i}\}_{i = 1}^{n - 1}, \{\theta_{1, i}\}_{i = n + 1}^{N}, \psi_{1} \mid Y_{1})
}.
\end{aligned}
\end{equation}
Now assume that $(\theta_{1, i} \indep \theta_{1, i^{'}}) \mid (\psi_{1}, Y_{1})$ for $i \neq i^{'}$ (I'm not sure this is true, but this at least seems feasible).
Ignoring the $\pd_{2}$ term for a moment, the remainder of the acceptance probability simplifies to
\begin{equation}
= \frac {
  \pd_{1}(\theta_{1, n}^{*} \mid \psi_{1}^{[t]}, Y_{1})
} {
  \pd_{1}(\theta_{1, n} \mid \psi_{1}^{[t]}, Y_{1})
}
\frac {
  \pd_{1}(\theta_{1, n} \mid \psi_{1}, Y_{1})
} {
  \pd_{1}(\theta_{1, n}^{*} \mid \psi_{1}, Y_{1})
}.
\end{equation}
We now have the interesting question of whether the terms in the target cancel with the terms in the proposal (due to the conditioning on $\psi_{1}^{[t]}$)?
Possibly, they do represent the same conditional distribution, though perhaps at different values?
