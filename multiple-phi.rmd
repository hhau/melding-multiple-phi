...
where $\overline{T}$ is the log of the crude event rate [@brilleman_bayesian_2020].

#### Third submodel

To avoid numerically integration in the second submodel, we model the longitudinal process via a hierarchical intercept model, using slightly different notation to distinguish it from submodel 1.
...
## Thoughts on modelling increasingly complex phenomena as a series of component models
