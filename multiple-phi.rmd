...
Unlike linear pooling, correlation present in $\pd_{2}$ will also persist in $\pd_{\text{pool}}$.
...
Our examples are necessarily simple, so that we can understand the impact of our choices on the pooled prior and provide useful advice on the specification process.

### Pooling Gaussian and Student-$t$ densities

#### Choosing relative values for the weights

We first investigate the choice of pooling method and value of $\lambda$.
Consider pooling following distributions
...
To understand the effect of $\lambda$ on $\pd_{\text{pool}}$ we set fix the parameters in the densities in Equation \eqref{eqn:marginal-gaussian-example}, and vary the value $\lambda$.
We set $\mu_{1} = -2, \mu_{2} = \left[\mu_{2, 1} \,\, \mu_{2, 2}\right]' = \left[0 \,\, 0\right]', \mu_{3} = 2, \sigma_{1}^{2} = \sigma_{2}^{2} = \sigma_{3}^{2} = 1$ and $\rho = 0.8$.
In the logarithmic case, we set $\lambda_{1} = \lambda_{3}$ and parameterise $\lambda_{2} = 1 - 2\lambda_{1}$, so that $\lambda_{1} + \lambda_{2} + \lambda_{3} = 1$ and we have only $\lambda_{1}$ to vary.
For the same reasons, in the linear case, we set $\lambda_{1, 1} = \lambda_{2, 2} = \lambda_{1}$ and $\lambda_{1, 2} = \lambda_{2, 1} = 1 - 2 \lambda_{1}$.
...
Logarithmic pooling does not display this behaviour.
For increasing values of $\lambda_{1}$ the variance of $\pd_{\text{pool}}$ consistently increases.
