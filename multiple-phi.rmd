...
It is in examples such as this one that we foresee the most use for chained Markov melding; a fully Bayesian approach is desired and the submodels are non-trivial in complexity, with no previously existing or obvious joint model.
...
This is possible due to the conditional independence between individuals in the posterior, and Appendix&nbsp;\ref{one-at-a-time} contains the details of this scheme.
...
\end{equation}
The model, detailed in Section \ref{pf-ratio-submodel-b-spline-pd_1}, uses the conditional independence between individuals to factorise such that
\begin{equation}
