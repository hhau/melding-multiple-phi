...
These authors collect and model three types of data: capture-recapture data $Y_{1}$ and capture-recapture submodel $\pd_{1}(\phi_{1 \cap 2}, \psi_{1}, Y_{1})$, population counts $Y_{2}$ and count data submodel $\pd_{2}(\phi_{1 \cap 2}, \phi_{2 \cap 3}, \psi_{2}, Y_{2})$, and nest-record data $Y_{3}$ and fecundity submodel $\pd_{3}(\phi_{2 \cap 3}, \psi_{3}, Y_{3})$.
...
Because of the shared parameters, we can form the IPM as the product of these distinct submodels.
...
Figure \ref{fig:owls-simple-dag} is a simplified DAG of the IPM, showing the relationships within the three submodels.
We will show that the chained Markov melding framework developed in Section \ref{model-specification} describes how this example, and IPMs in general, are integrated.
