...
- come back post rewrite subsections
<!-- - the thing we want to say here is that these examples are illustrative applications of our methodology. 
    - In what way are they illustrative? why are we considering them?
      - The first one is illustrative because it shows that 'chains of submodels' is not just a made up structure. There are uses for this type of thinking.
        - It would be nice to find an example of something that changed between the separate submodel analysis and the integrative one.
        - This is what Rob was talking about with the change in prior between the two settings.
          - not super hopeful here, ecology seems fairly adverse to informative priors when using IMPs. Those times when they do use an informative prior, it is for a specific case (e.g. known sex imbalance) and remains so in the IPM (it is a specific prior on an element in $\psi$)
      - The second one is a little more tricky, but effectively the argument is that we want to do survival analysis where we have two longitudinal processes. We model the first one and derive our event time, and we think it's related to the second one. So we're doing joint models. The key difference is that the longitudinal processes are noisy, so the derived event times are also noisy.
 -->
We illustrate our methodology using two examples in which chains of submodels arise in the modelling process.
