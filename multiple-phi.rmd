...
Our extension addresses previously unconsidered complications including the distinct domains (and possibly supports) of the common quantities, and the desire to capture possible prior correlation between $\phi_{1 \cap 2}$ and $\phi_{2 \cap 3}$.
...
We will provide more details and illustrate our methodology with these examples in Sections \ref{an-integrated-population-model-for-little-owls-1} and \ref{survival-analysis-with-time-varying-covariates-and-uncertain-event-times-1}.
...
This relatively infrequency of P/F ratio data, when combined with the intrinsic variability in each individual's blood oxygen level, results in significant uncertainty in about the time of onset of RF.
...
However, existing joint models are not able to incorporate the uncertainty surrounding the event time, which may result overconfident and/or biased estimates of the parameters in the joint model.
...
Define the vector of all common quantities $\boldsymbol{\phi} = (\phi_{1}, \ldots, \phi_{\Nm}) = (\phi_{1 \cap 2}, \ldots, \phi_{\Nm - 1 \cap \Nm})$, and denote by $\boldsymbol{\phi}_{-\modelindex}$ the subvector of $\boldsymbol{\phi}$ excluding the $\modelindex$\textsuperscript{th} element.
...
Selecting a specific form of $g$ is not trivial given the many choices of functional form and pooling weights (the latter we discuss momentarily).
...
with $K_{\text{log}}(\lambda) = \int \prod_{\modelindex = 1}^{\Nm} \pd_{\modelindex}(\phi_{\modelindex})^{\lambda_{\modelindex}} \text{d}\boldsymbol{\phi}$ for nonnegative weight vector $\lambda = (\lambda_{1}, \ldots, \lambda_{\Nm})$.
...
If two consecutive marginals are chosen to have the same submodel prior then we instead define
...
Whilst the melded posterior is a standard Bayesian posterior and so can, in principle, be targetted using any suitable Monte Carlo method, in practice this may be cumbersome or infeasible.
...
<!-- \begin{itemize}[nosep]
  \tightlist
  \item Such approximations are appropriate for $\pd_{1}(\phi_{1 \cap 2}), \pd_{1}(\phi_{1 \cap 2} \mid Y_{1}), \pd_{3}(\phi_{2 \cap 3})$, and $\pd_{3}(\phi_{2 \cap 3} \mid Y_{3})$.
  \item We are not interested in $\psi_{1}$ and $\psi_{3}$, and can integrate them out of all relevant densities.
  \item We employ our second form of dictatorial pooling and choose $\pd_{2}(\phi_{1 \cap 2}, \phi_{2 \cap 3})$ as the authoritative prior.
\end{itemize} \noindent -->
...
Our intention is to illustrate the application of chained Markov melding to an example of realistic complexity, and explore empirically the importance of accounting for all sources of uncertainty by comparing chained Markov melding to equivalent analyses that use only a point estimate summary of the uncertainty.
...
The rate of fluid administration reflects the clinical management of patients by ICU staff, and hence changes to the rate reflect a decision to change treatment strategy.
Thus the cumulative fluid balance primarily reflects these decisions rather than the underlying physiological state of a patient.
...
Models with more breakpoints, or a variable number of breakpoints, are possible but their complexity puts them beyond the scope of this example.
...
The left column of Figure \ref{fig:pf_fit_and_fluid_fit_plot} displays the P/F data, the fitted submodel, and derived event times for individuals $i = 17$ and $29$.
...
The right column of Figure \ref{fig:pf_fit_and_fluid_fit_plot} displays the cumulative fluid data and the fitted submodel, with the little noise in the data resulting in minimal uncertainty about the fitted value and a concentrated subposterior distribution.
...
Our currently sampler is also sensitive to large differences in location or scale of the target distribution between the stages.
The impact of these differences can be ameliorated using @manderson_numerically_2021, and, more generally, Sequential Monte Carlo samplers are likely to perform better in these settings.
...
However, the general and Bayesian nature of our methodology readily facilitates any form of uncertainty in the event times and the event indicators; uncertainty in the latter is not considered the cited papers.
...
\end{equation}
The model, detailed in Section \ref{pf-ratio-submodel-b-spline-pd_1}, factorises such that
\begin{equation}
