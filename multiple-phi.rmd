...
[^chained]: _'Chained graphs'_ were considered by @lauritzen_chain_2002 however they are unrelated to the form of our proposed model. We use 'chained' to emphasise the nature of the relationships between the submodels.
...
We will show that the form proposed in Equation \eqref{eqn:melded-model-full} satisfies this property, whilst the model produced by applying the melding method of @goudie_joining_2019 is sensitive to the order of integration.
    <!-- 
...
We define the parameter space of the output as $\boldsymbol{\Theta}_{12} = (\phi_{1 \cap 2}, \phi_{2 \cap 3}, \psi_{1}, \psi_{2}, Y_{1}, Y_{2})$, so that any prior marginal distribution of $\pd_{\text{meld}}^{12}$ can be derived by integrating out the irrelevant parts of $\boldsymbol{\Theta}_{12}$.
For example,  
...
so that the parentheses in the superscript of $\pd_{\text{meld}}^{(12)3}$ indicate the order in which the melding operator is applied.
...
In the following sections it will be convenient to expand Equation to \eqref{eqn:iterated-application-melding-two} to
...
- Question as to whether we introduce censoring? Probably yes once this works and is written.
...
- Similar plot for $\phi_{2 \cap 3}$? Or work into simplified version of Figure \ref{fig:phi_12_contraction}.
- We can see from Tables \ref{tab:surv-stage-one-submodel-three} and \ref{tab:surv-stage-two-diag-phi-23} that there is also no contraction for $\phi_{2 \cap 3}$. Interesting. Need to think about.
