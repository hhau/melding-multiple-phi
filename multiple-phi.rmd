...
- @brilleman:etal:20 note that _"However, in general, there is no longer a closed form expression for the cumulative hazard, survival function, or CDF. Therefore, when the linear predictor includes time-varying coefficients, quadrature is used ..."_, which is a problem. We may need to get the Stan code used to do the quadrature?
    ...
    - looks like @austin:12 has done the integration for us?
...
- the Multi-stage sampler seems to be doing a very similar thing to the correction proposed in @mauff:etal:2020 in Eq (6).
...
- What on earth is $\pd_{2}(\phi_{1 \cap 2}, \phi_{2 \cap 3})$?
    ...
    - there is a suite a methodology for simulating realistic event times from a survival model i.e. @crowther:lambert:13. Can we repurpose this to sample from the prior? I think this is what @brilleman:simsurv:19 uses.
