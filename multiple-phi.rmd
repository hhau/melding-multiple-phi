...
- Developing models for numerous, disparate data sources is challenging.
- An easier strategy may be to model each data source individually, then coherently combine these models.
- @goudie:etal:18 propose Markov melding, a method for combining submodels that share a single common quantity $\phi$.
- We extend Markov melding to allow us to compose submodels that have quantities in common in a pairwise manner.

- The simplest example is the 3 submodel case (ILLUSTRATED IN FIG TBA), where $\pd_{1}(\phi_{1 \cap 2}, \psi_{1}, Y_{1})$ and $\pd_{2}(\phi_{1 \cap 2}, \phi_{2 \cap 3}, \psi_{2}, Y_{2})$ have $\phi_{1 \cap 2}$ in common, retaining submodel specific parameters and data $(\psi_{1}, Y_{1})$ respectively.
- The additional third submodel $\pd_{3}(\phi_{2 \cap 3}, \psi_{3}, Y_{3})$ shares $\phi_{2 \cap 3}$ with the second submodel, whilst also having submodel specific parameters and data.

<!-- This need refining -->
- thinking of melding in the manner we propose is coherent, in contrast to applying melding twice (execpt in cases of independence).
- It also enables a more parallelised sampling process.
