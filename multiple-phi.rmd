...
csl: bibliography/aam71-test.csl
output: 
  pdf_document:
    includes:
      in_header:
...
Forming $\pd_{\text{pool}}(\phi_{1:2}, \phi_{2:3})$ requires us to coherently combine $\pd_{1}(\phi_{1:2}), \pd_{2}(\phi_{1:2}, \phi_{2:3})$, and $\pd_{2}(\phi_{2:3})$, with corresponding weights $\lambda_{1}, \lambda_{2}, \lambda_{3} \geq 0$ .
...
If $\phi_{1:2} \in \mathbb{R}^{d_{1}}$ and $\phi_{2:3} \in \mathbb{R}^{d_{2}}$, then the supports of $\pd_{1}, \pd_{2}$, and $\pd_{3}$ are $\mathbb{R}^{d_{1}}, \mathbb{R}^{d_{1} \times d_{2}}$, and $\mathbb{R}^{d_{2}}$ respectively.
Because the supports differ for each marginal, an additive combination is incoherent. 
Hence the natural extension to linear pooling
\input{tex-input/multiple-phi/0052-linear-pooling.tex}
does not admit a valid probability density function.
...
then forming the pooled prior as
\input{tex-input/multiple-phi/0055-silly-linear-overall.tex}
We do not need any weights in Equation \eqref{eqn:silly-linear-overall}, as $\pd_{\text{pool}, 1}$ and $\pd_{\text{pool}, 2}$ are independent of each other.
However, this process will always produce a pooled prior with no correlation between $\phi_{1:2}$ and $\phi_{2:3}$, which may be undesirable.
...
This is a coherent probability density function for $\phi_{1:2}, \phi_{2:3}$, conditional on $\pd_{\text{pool}}$ being integrable.
...
The first is an equal apportionment $\lambda = (\frac{1}{3}, \frac{1}{3}, \frac{1}{3})$, whilst the second is an unequal distribution $\lambda = (\frac{1}{9}, \frac{7}{9}, \frac{1}{9})$ meant to account for the higher dimensional nature of $\pd_{2}(\phi_{1:2}, \phi_{2:3})$.
...
- We could distribute weight between $(p_{1}(\phi_{1:2}) p_{3}(\phi_{2:3}))$ and $\pd_{2}(\phi_{1:2}), \phi_{2:3})$, as these are equal dimensional objects.
