...
## Integrated population models - Owls
...
- Some notation is common to all submodels. Data and parameters are stratified into
    - Two age-groups $a \in \{J, A\}$ ($J$: juvenile, $A$: adult),
    - Two sexes $s \in \{M, F\}$,
    - Time $t \in \{1, \ldots, T\}$, for $T = 25$.
...
- The count data model is $\pd_{2}$ 
    - $\phi_{1 \cap 2} = \phi_{a, g, t} = (\alpha_{0}, \alpha_{1}, \alpha_{2})$
        - Count data is female specific, and $\alpha_{2}$ is a male-specific term, so it doesn't exist in the count data model.
    - $\phi_{1 \cap 2} = \rho$
...
A simple Poisson models is employed, with fecundity rate $\rho$, as a model for these data
\input{tex-input/owls-example/0030-fecundity-submodel.tex}
<!-- Following @finke:etal:19 we specify a constant value for $\rho$ over all time points. -->
...
```{r stage_two_mcmc_diags, fig.cap = "stage two visual diagnostics."}
