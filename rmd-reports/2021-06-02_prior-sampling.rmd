...
- This was more of a pain than I thought due to the exact zeros and ones that arise due to rescaling and the definition of the death / discharge event.
...
```{r beta_mix}
knitr::include_graphics('../plots/mimic-example/pf-prior-plot.png')
```
...
- Quite often, this is a good fit:
```{r p2_good_fit}
knitr::include_graphics('../plots/mimic-example/temp-pairs/pairs-04.png')
```
- Sometimes, the optimizer throws a wobbly and returns something nonsensical:
```{r p2_bad_fit}
knitr::include_graphics('../plots/mimic-example/temp-pairs/pairs-06.png')
```
- Throwing more prior samples into the optimizer might be helpful, because I suspect the wacky output is due to class imbalance.
    ...
    - Current 'solution' is a `while` loop that restarts the optimiser at new initial values.
- At the moment, I am not sampling the event types in the correct way [@beyersmann_simulating_2009], but our setting is really competing risks in a way -- the second event can only happen at death/discharge, and for the supports of $\pd_{1}(\phi_{1 \cap 2})$ and $\pd_{2}(\phi_{1 \cap 2})$ to match, the survival submodel needs to know about the length of stay.
