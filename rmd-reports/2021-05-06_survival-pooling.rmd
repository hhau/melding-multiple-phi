...
The internal knots evenly spaced between the two boundary knots at $\min(\boldsymbol{t_{i}})$ and $\max(\boldsymbol{t_{i}})$.
...
We denote the individual specific B-spline basis evaluated at time $t_{i, j}$ as $B_{i}(t_{i, j}) \in \left\{\mathbb{R}_{+} \cup \{0\}\right\}^{10}$ so that the submodel can be written as
\begin{equation}
...
If there are no roots then the individual died or was discharged before respiratory failure occurred so we set $T_{i} = C_{i}$ and $d_{i} = 2$.
...
\end{equation}
The resulting density estimate, mixture weight $\widehat{\theta}_{i} \in [0, 1]$, is
\begin{align*}
