...
we use for this submodel/example, as \texttt{NIMBLE} will sample a node
\texttt{x} defined as \texttt{x\ \textasciitilde{}\ dpois(0.0)} with all
samples being \(0\).
...
\NormalTok{samples\_nimble }\OtherTok{\textless{}{-}}\NormalTok{ nimble}\SpecialCharTok{::}\FunctionTok{nimbleCode}\NormalTok{(\{}
...
\FunctionTok{as.numeric}\NormalTok{(samples\_nimble)}
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\centering
\includegraphics{plots/indiv-18-subset.png}
\caption{Pairs plot for individual \(i = 18\) in Example 2. The
displayed are parameters are \(\beta_{0, i}\) and
\(\xi_{i, 1}, \ldots, \xi_{i, 3}\).}
\end{figure}
