...
simulated_data <- with(sim_settings, 
  bind_rows(lapply(1 : n_patients, function(patient_id) {
    ...
    beta_one_true <- long_rand_slope[patient_id]

    obs_times <- sort(runif(
      n = n_obs_per_patient[patient_id]
    ))

    true_values <- beta_zero_true + obs_times * beta_one_true
    obs_values <- abs(
      true_values + 
      rnorm(n = n_obs_per_patient[patient_id], mean = 0, sd = 10 * sigma_noise)
    )
    res <- tibble(
