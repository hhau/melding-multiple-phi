...
flog.info("MIMIC-stage-two-poe-fix-phi-12-meld-phi-23: compiling models", name = base_filename)
...
list_res <- mclapply(1 : n_stage_two_chain, mc.cores = 5, function(chain_id) {
  ...
  flog.info(
    sprintf("MIMIC-stage-two-poe-fix-phi-12-meld-phi-23--chain-%d: initialising", chain_id),
    name = base_filename
  )
  ...
  flog.info(
    sprintf("MIMIC-stage-two-poe-fix-phi-12-meld-phi-23--chain-%d: initialised okay", chain_id),
    name = base_filename
  )
  ...
  for (ii in 2 : n_stage_two_iter) {
    ...
    if (ii %% 500 == 0) {
      flog.info(
        sprintf("MIMIC-stage-two-poe-fix-phi-12-meld-phi-23--chain-%d: iteration-%d", chain_id, ii),
        name = base_filename
      )
    }
  }
...
flog.info(
  "MIMIC-stage-two-poe-fix-phi-12-meld-23: writing to disk",
  name = base_filename
)
