...
flog.info("MIMIC-stage-two-logarithmic-meld-phi-12-fix-phi-23: compiling models", name = base_filename)
...
list_res <- mclapply(1 : n_stage_two_chain, mc.cores = 5, function(chain_id) {
  ...
  flog.info(
    sprintf("MIMIC-stage-two-logarithmic-meld-phi-12-fix-phi-23--chain-%d: initialising", chain_id),
    name = base_filename
  )
  ...
  flog.info(
    sprintf("MIMIC-stage-two-logarithmic-meld-phi-12-fix-phi-23--chain-%d: initialised okay", chain_id),
    name = base_filename
  )
  ...
  for (ii in 2 : n_stage_two_iter) {
    ...
    for (jj in 1 : n_icu_stays) {
      ...
      log_prob_pooled_prior_prop <- logarithmic_pooled_prior_meld_phi_12_fix_phi_23_overall(
        phi_12 = as.numeric(phi_12_indiv_prop_list),
        lambda = logarithmic_lambda_vec,
        id = indiv_index
      )

      log_prob_pooled_prior_curr <- logarithmic_pooled_prior_meld_phi_12_fix_phi_23_overall(
    ...
    if (ii %% 500 == 0) {
      flog.info(
        sprintf("MIMIC-stage-two-logarithmic-meld-phi-12-fix-phi-23--chain-%d: iteration-%d", chain_id, ii),
        name = base_filename
      )
    }
  }
...
flog.info("MIMIC-stage-two-logarithmic-meld-phi-12-fix-phi-23: writing to disk", name = base_filename)
