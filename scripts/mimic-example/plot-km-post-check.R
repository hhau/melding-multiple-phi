...
psi_2_poe_samples <- readRDS("rds/mimic-example/stage-two-poe-psi-2-samples.rds")
phi_23_poe_samples <- readRDS("rds/mimic-example/stage-two-poe-phi-23-samples.rds")
phi_12_poe_samples <- readRDS("rds/mimic-example/stage-two-poe-phi-12-samples.rds")

psi_2_fixed_samples <- readRDS("rds/mimic-example/stage-two-median-inputs-psi-2-samples.rds")
phi_12_point_est <- readRDS("rds/mimic-example/median-event-time-data.rds")
phi_23_point_est <- readRDS("rds/mimic-example/median-fluid-fit-data.rds")
...
res_fixed <- rbind(base_df_fixed, surv_obj_fixed) %>% 
  mutate(origin = 'stage_one_median_phi_12', origin_alpha = 0.8)

plot_df <- bind_rows(res) %>% 
  mutate(origin = 'stage_two_poe_phi_12', origin_alpha = 0.025) %>% 
  bind_rows(res_fixed)
...
base_plot <- ggplot(
  ...
  geom_step(aes(alpha = origin_alpha)) + 
  geom_point(
    data = censor_df,
    inherit.aes = FALSE,
...
ggsave_base(
  filename = "plots/mimic-example/kaplan-meier-pc.png",
  plot = p1,
  width = display_settings$half_page_plot_width + 15,
